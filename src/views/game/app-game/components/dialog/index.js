...
export default {
  ...
  methods: {
    ...
    // 提交校验
    $_submitVerify() {
      const fileListArr = []; const swiperListArr = []
      this.fileList.map(item => fileListArr.push({ screenshot: item.url, name: item.name, type: item.type }))
      this.swiperList.map(item => swiperListArr.push({ screenshot: item.url, name: item.name, type: item.type }))
      this.editForm.imgBase_64 = fileListArr
      this.editForm.swiperListBase_64 = swiperListArr
    }
